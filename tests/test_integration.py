...
def test_agent():
    ...
    assert (
        response.answer is not None and len(response.answer.strip()) > 0
    ...
    response = agent.process_input(
        "Summarize the following text about artificial intelligence..."
    )
    print(f"Answer: {response.answer}")
    print(f"Sources: {response.sources}")
    ...
    assert (
        "what is the capital of france" in response.answer.lower()
    ), "Should remember previous question"
    ...
    assert (
        response.answer == "50"
    ), f"Calculator tool should work, got '{response.answer}'"
...
def test_error_handling():
    ...
    for i, case in enumerate(edge_cases, 1):
        ...
        try:
            ...
            print(
                f"Response: {response.answer[:50]}{'...' if len(response.answer) > 50 else ''}"
            )
